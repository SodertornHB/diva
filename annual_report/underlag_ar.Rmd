...
diva <- diva %>% filter(between(Year, 2013, 2016))
```
```{r ahead, echo=FALSE}
...
rev <- nrow(subset(diva, Reviewed =="false"))
```
...
Publikationstyperna "Redaktörskap (Proceedings och Samlingsverk)" och "Övrigt" är inte medräknade.
...
Tabellerna till årsredovisningen finns i bifogad fil.
...
ar_table <- ar_table[c(1,5,4,3,2)]
ar_table[is.na(ar_table)] <- 0L

# Följande gör berörda kolumner till double istället för integer ar_table[is.na(ar_table)] <- 0
