#
#
#Öppen publicering SH
#171221 GL
#Sammanställning av olikat typer av OA för SH-publikationer.
#
#
...
stats_oa_book <- full_join(stats_oa_book, stats_unique_book, "Year")
...
sh_archive_df(counts_oa, "Antal_oa.csv")
sh_archive_df(shares_oa, "Andel_oa.csv")
sh_archive_df(counts_tot, "Tot_pub.csv")
sh_archive_end()
...
library(ggplot2)
ggplot() + 
  geom_line(data = stats_shares, aes(Year, percent_doaj), colour = "darkgoldenrod") +
  geom_line(data = stats_shares_kap, aes(Year, percent_freefulltext_kap), colour = "blue4") +
  geom_line(data = stats_shares, aes(Year, percent_freefulltext), colour = "deeppink") +
  geom_line(data = stats_shares, aes(Year, percent_divafulltext), colour = "darkgreen") +
  labs(title ="Öppen tillgång till artiklar och kapitel publicerade av forskare vid Sh. Artiklar kan finnas i flera linjer.",
       x = "Publiceringsår",
       y = "Andel öppet tillgängliga publ i %") +
  theme_bw() +
  theme(plot.title = element_text(size = 8))

ggplot() + 
  geom_line(data = stats_shares, aes(Year, percent_doaj), colour = "yellow") +
  #geom_line(data = stats_shares_kap, aes(Year, percent_freefulltext_kap), colour = "green") +
  geom_line(data = stats_shares, aes(Year, percent_freefulltext), colour = "pink") +
  geom_line(data = stats_shares, aes(Year, percent_divafulltext), colour = "green") +
  labs(title ="Öppen tillgång till publikationer från Sh",
       x = "Publiceringsår",
       y = "Andel öppet tillgängliga publ i %")
